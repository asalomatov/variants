...
ftrs.variants.head()
df_l = []
for f in df_train['family_id'][:10]:
    print f
    fv = reload(features_vcf)
    train_set = "/mnt/scratch/asalomatov/data/SSC/SSCdeNovoCalls/ssc_exome_verified_snp.txt"
    ftrs = fv.FeaturesVcf(myped, train_set) 
...
def addVar(df, field):
    try:
        df['var'] = df[field].map(str) + '_' + df.POS.map(str) + '_' +  df.REF.map(str)  + '_' +  df.ALT.map(str) 
    except:
        df['var'] = df[field].map(str) + '_' + df.pos.map(str) + '_' +  df.ref.map(str)  + '_' +  df.alt.map(str) 
    return df
...
def df2sklearn(mydf):
    ...
    col_to_keep = [u'QUAL', u'info_BaseQRankSum', u'info_ClippingRankSum', u'info_DP', u'info_FS', u'info_GC', u'info_HRun',
            ...
            u'format_offspring_0_PL', u'format_offspring_1_PL', u'format_offspring_2_PL', u'status01']
...
df_train_num = df2sklearn(df_train)
df_train_num.isnull().sum()
df_train_num.head()
df_train_num.dtypes

df_neg.isnull().sum()
df_neg_num = df2sklearn(df_neg)
