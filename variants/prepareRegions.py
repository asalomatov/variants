...
print sys.argv
if len(sys.argv) != 3:
    sys.exit('two argument required: child_id')
child_id = '11006.p1'
caller = 'fb'
child_id = sys.argv[1]
caller = sys.argv[2]
...
vcf_pat = ''
if caller == 'hc':
    vcf_pat = '/mnt/scratch/asalomatov/data/SSC/wes/vcf/hc/%s.family.vqsr.sorted.vcf'
elif caller == 'fb':
    vcf_pat = '/mnt/scratch/asalomatov/data/SSC/wes/vcf/fb/%s.family.freebayes.sorted.vcf'
else:
    sys.exit('unknown caller, exiting... ')

output_dir ='/mnt/scratch/asalomatov/data/SSC/wes/feature_sets/' + caller
...
if not f.initTrioFor(child_id):
    ...
    fam_features = pool.map(multi_wrap_readBamReadcount, \
        [(f.sample_features, var_type), (f.father_features, var_type), (f.mother_features, var_type)])
    f.removeTmpDir()
...
# add status, and extra fields then save to file
    ...
    clmns_to_drop = [ 'REF_offspring', 'DP_offspring', 'REF_avg_se_mapping_quality_offspring', 
            'REF_avg_se_mapping_quality_offspring',
            'REF_father', 'DP_father', 'REF_base_father', 'REF_avg_se_mapping_quality_father',
            'ALT_base_father', 'REF_avg_se_mapping_quality_father',
            'REF_mother', 'DP_mother', 'REF_base_mother', 'REF_avg_se_mapping_quality_mother',
            'ALT_base_mother', 'REF_avg_se_mapping_quality_mother', 'vartype']
